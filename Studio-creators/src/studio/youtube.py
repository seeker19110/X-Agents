...
def load_client_secrets(path: Path) -> tuple[str, str]:
    ...
    if not cid or not csec:
        raise PlatformError(
            f"{path}: không có client_id/client_secret (tải JSON OAuth 'Desktop app' từ Google Cloud Console)"
        )
...
def exchange_code(
    ...
    if st != 200:
        raise PlatformError(f"đổi mã thất bại HTTP {st}: {raw[:200].decode('utf-8', 'replace')}", status=st)
    d = json.loads(raw)
    if not d.get("refresh_token"):
        raise PlatformError(
            "Google không trả refresh_token — thu hồi quyền ứng dụng ở myaccount.google.com/permissions rồi đăng nhập lại"
        )
    ...
    return Tokens(
        ...
        client_secret=client_secret,
        expiry=exp,
        scopes=(d.get("scope") or " ".join(SCOPES)).split(),
    )
...
def _wait_for_code(port: int, state: str, timeout_s: float = 300.0) -> str:
    ...
    class H(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(
                ("<h3>Đăng nhập YouTube xong, đóng tab này.</h3>" if ok else "<h3>Thiếu code/state sai.</h3>").encode()
            )
            done.set()
    ...
    t.start()
    try:
        if not done.wait(timeout_s) or "code" not in got:
            raise PlatformError("không nhận được mã OAuth (hết giờ hoặc state sai)")
        return got["code"]
    finally:
        srv.shutdown()
        srv.server_close()
...
def login(
    ...
    print(
        f"Mở trình duyệt để cấp quyền ({', '.join(s.rsplit('/', 1)[-1] for s in SCOPES)}). Nếu không tự mở, dán URL:\n{url}"
    )
    if open_browser:
        webbrowser.open(url)
    code = _wait_for_code(port, state)
    t = exchange_code(code, cid, csec, redirect, fetcher)
    store.save(t)
...
def find_ref(bus: Any, video_id: str) -> str | None:
    for e in reversed(list(bus.replay("publish-events", video_id))):
        if (
            e.payload.get("kind", "video") == "video"
            and e.payload.get("platform_ref")
            and e.payload.get("status") in {"scheduled", "published"}
        ):
...
def find_channel(bus: Any, video_id: str) -> str:
    for e in bus.replay("video-briefs", video_id):
        if e.payload.get("channel_id"):
            return str(e.payload["channel_id"])
    return ""


def _audit(bus: Any, action: str, video_id: str, data: dict[str, Any]) -> None:
    a = AuditLog(
        actor=ACTOR, action=action, video_id=video_id, evidence=json.dumps(data, ensure_ascii=False, default=str)[:4000]
    )
...
def sync_comments(
    bus: Any, platform: Platform, video_id: str, ref: str | None = None, since: str | None = None
) -> Envelope | None:
    ref = ref or find_ref(bus, video_id)
    if not ref:
        raise PlatformError(f"{video_id}: chưa có platform_ref (chưa upload?) — truyền --ref")
...
def sync_metrics(
    ...
    _audit(
        ...
        video_id,
        {
            ...
            "retention_points": len(snap.retention_curve),
        },
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="file token (mặc định ~/.x-agents/auth/youtube_tokens.json hoặc STUDIO_YOUTUBE_TOKENS)",
    )
    sub = ap.add_subparsers(dest="cmd", required=True)
    lg = sub.add_parser("login")
    lg.add_argument("--client-secrets", type=Path, required=True)
    lg.add_argument("--port", type=int, default=8765)
    lg.add_argument("--no-browser", action="store_true")
    sub.add_parser("status")
    sc = sub.add_parser("sync-comments")
    sc.add_argument("video_id")
    sc.add_argument("--ref", default=None)
    sc.add_argument("--since", default=None)
    sm = sub.add_parser("sync-metrics")
    sm.add_argument("video_id")
    sm.add_argument("--ref", default=None)
    sm.add_argument("--window", type=int, default=7)
    sm.add_argument("--channel", default=None)
    sm.add_argument("--variant", default=None)
    ns = ap.parse_args(argv)
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8")
    cfg = load_media_config()
    store = TokenStore(ns.tokens or _token_path(cfg))
    try:
        if ns.cmd == "login":
            login(ns.client_secrets, store, port=ns.port, open_browser=not ns.no_browser)
            return 0
        if ns.cmd == "status":
            print(
                json.dumps(
                    {"provider": cfg.platform.get("provider", "fake"), **store.status()}, ensure_ascii=False, indent=2
                )
            )
            return 0
        ...
        if ns.cmd == "sync-comments":
            ...
            print(
                f"{platform.name}: {len(env.payload['comments']) if env else 0} bình luận"
                + (f" → audience-comments event={env.event_id}" if env else " (không có gì mới)")
            )
            ...
            env = sync_metrics(
                ...
                channel_id=ns.channel,
                variant_id=ns.variant,
            )
            p = env.payload
            print(
                f"{platform.name}: views={p['views']} impressions={p['impressions']} ctr={p['ctr']} avd={p['avg_view_duration_s']}s "
